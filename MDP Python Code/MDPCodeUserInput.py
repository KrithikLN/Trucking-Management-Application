...
states = ['Chennai', 'Madurai', 'Coimbatore', 'Loading', 'Unloading', 'En route to Destination', 'Waiting', 'Breakdown',
          'Refueling', 'Traffic Congestion', 'Detour', 'Weather Conditions']
...
def simulate_model(num_episodes=100):
    ...
    for _ in range(num_episodes):
        ...
        # Ask for the initial shed
        print("Select the initial shed (Enter 1 for Chennai, 2 for Madurai, 3 for Coimbatore):")
        initial_shed_choice = int(input())
        initial_shed = states[initial_shed_choice - 1]
        state = states.index(initial_shed)  # Convert shed to index
        driver = np.random.choice(len(drivers))  # Assign a random driver

        while True:
            action = choose_action(state)  # Choose action using learned policy

            if state == states.index('Refueling'):  # Check if the truck is in the refueling state
                next_state_index = states.index(initial_shed)  # Truck returns to the initial shed after refueling
            elif state in [0, 1, 2]:  # Check if the truck is in one of the sheds
                print("Select the next state (Enter the index from 3 to 11):")
                next_state_index = int(input())  # User input for next state index
            elif state in [3, 4, 5, 6, 7]:  # Check if the truck is in a shed or loading/unloading
                next_state_index = driver  # Truck goes back to its starting shed
            else:
                print("Select the next state (Enter the index from 0 to 11):")
                next_state_index = int(input())  # User input for next state index

            if next_state_index in [0, 1, 2]:  # Check if the truck returns to its starting shed
                break

            next_state = states[next_state_index]

            # Reward is manually provided by the user
            reward = int(input(f"Enter reward for transition from {states[state]} to {next_state}: "))
...
for episode in range(num_episodes):
    state = states.index('Refueling')  # All trucks start by refueling
    driver = np.random.choice(len(drivers))  # Assign a random driver
    ...
    while True:
        ...
        update_Q(state, driver, action, reward, next_state_index)
